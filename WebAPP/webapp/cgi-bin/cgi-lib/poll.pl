...
~;
		...
		print qq~<form action="$cgi?action=pollit" method="post">
...
<input type="submit" class="votebutton" value="$btn{'002'}"><br><br>
...
##############
	...
	open('LOG', "$datadir/polls/$input{'id'}_ip.dat") || error("$err{'001'} $datadir/polls/$id[0]_ip.dat");
	lock(LOG);
	@entries = <LOG>;
	unlock(LOG);
	close(LOG);
