...
# ***************************************
# 1st September 2001
# Those new codes commented by # cnctek
# are introduced by cnctek.com to support
# the operation of the "uploader.cgi"
# script only.
# ***************************************
...
$cgi_lib'maxdata    = 2097152;    # maximum bytes to accept via POST - 2^21
...
sub ReadParse {
  ...
  # if ($len > $cgi_lib'maxdata) { # cnctek
  #     &CgiDie("cgi-lib2.pl: Request to receive too much data: $len bytes\n"); # cnctek
  # } # cnctek
  ...
  if (!defined $meth || $meth eq '' || $meth eq 'GET' || 
    ...
    if (!defined $meth || $meth eq '') {
      ...
      &CgiDie("cgi-lib2.pl: Unknown request method: $meth\n");
    }
   ...
   PART: # find each part of the multi-part while reading data
    while (1) {
     ...
     BODY: 
	  ...
	  &CgiDie("cgi-lib2.pl: reached end of input while seeking boundary " .
	...
	die "Short Read: wanted $amt, got $got\n" if $errflag;
     ...
     HEAD:
      ...
      if ($incfn{$name}) { # cnctek
        $source_filename = $incfn{$name}; # cnctek
        $source_filename =~ s/[:\\]/\//g; # cnctek
        $source_filename =~ s/.*\///g; # cnctek
      } # cnctek
...
END_MULTIPART
    if ($errflag) {
      ...
      if (!$in{'destn_filename'} || $in{'destn_filename'} =~ /^[ ]+$/) { # cnctek
        if ($source_filename) { # cnctek
          $in{'destn_filename'} = $source_filename; # cnctek
        } # cnctek
      } # cnctek
    ...
    &CgiDie("cgi-lib2.pl: Unknown Content-type: $ENV{'CONTENT_TYPE'}\n");
  }
