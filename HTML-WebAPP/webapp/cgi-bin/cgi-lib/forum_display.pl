...
# forum_display.pl    	                                                      #
# v0.9.9 - Requin                                                             #
# Date Formatting, Timezones & Print Thread Improvements by: Floyd for v0.9.8 #
#                                                                             #
...
foreach $curcat (@categories) {
	...
	if ($catinfo[1] ne "") {
		...
		if ($currentboard eq "$searchcat" && $settings[7] ne "$catinfo[1]") {
...
~;
	...
	if($username eq "$boardmoderator" || $settings[7] eq "$root") { $adminfunction = qq~<a href="$forum&amp;op=movestickythread&amp;thread=$viewnum"><img src="$imagesurl/forum/move.gif" alt="$msg{'113'}" border="0"></a>&nbsp;<a href="$forum&amp;op=removestickythread&amp;thread=$viewnum"><img src="$imagesurl/forum/remove.gif" alt="$msg{'114'}" border="0"></a>&nbsp;<a href="$forum&amp;op=lockstickythread&amp;thread=$viewnum"><img src="$imagesurl/forum/lock.gif" alt="$msg{'115'}" border="0"></a>~; }
...
print qq~</td>
<td colspan="2" align="right" valign="top" class="forumtext"><form action="$cgi" method="get">
...
print qq~</td>
<td align="right" colspan="2" class="forumtext"><form action="$cgi" method="get">
$msg{'104'} <select name="board">$selecthtml</select> <input type="submit" class="button" value="$btn{'014'}">
<input type="hidden" name="action" value="forum"></form></td>
</tr>
</table>
...
############ 
     ...
     foreach $jumpfile (@jumptofiles) { 
           ...
           $jumpinfo[1] =~ s/[\n\r]//g; 
           if ($jumpinfo[1] ne "") { 
                 if ($settings[7] ne "$root" && $settings[7] ne "$jumpinfo[1]") {      next; } 
           } 
