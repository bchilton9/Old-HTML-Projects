...
elsif ($INPUT{'a'} eq "memlist") { &memlist }
...
elsif ($INPUT{'a'} eq "valt") { &valt; }
elsif ($INPUT{'a'} eq "valtadd") { &valtadd; }
elsif ($INPUT{'a'} eq "Add_Item") { &valtadditem; }
elsif ($INPUT{'a'} eq "valtform") { &valtform; }
elsif ($INPUT{'a'} eq "Delete_from_Valt") { &valtdelete; }
elsif ($INPUT{'a'} eq "Edit_Valt_Item") { &valtedita; }
elsif ($INPUT{'a'} eq "Save_Valt_Item") { &valteditb; }
elsif ($INPUT{'a'} eq "valt+") { &valtplus; }
elsif ($INPUT{'a'} eq "valt-") { &valtmins; }
...
elsif ($INPUT{'a'} eq "Delete_Message") { &delete_mess }
elsif ($INPUT{'a'} eq "Reply") { &send }
elsif ($INPUT{'a'} eq "Send Message") { &sentmessage }
...
$visit = "$visit <A HREF=?a=Reply&sender=$name id=visitlist>$name</A>,";
...
if ($acount eq "Silver") {
$mem = "$mem<TD><div id=text3>Gold Only</A></TD>";
}
else {
...
if ($acc eq "Master") {
$mem = "$mem<TD><div id=text3>Master</div></TD>";
}
else {
...
sub page {
if ($acount eq "Silver") {
$cont = "<DIV id=text1>Gold Acount Only</DIV>";
}
else {
...
sub photolist {
if ($acount eq "Silver") {
$plist = "<div id=text1>Gold Acount Only</DIV>";
}
else {
open (DATA, "photo.lst");
	@data = <DATA>;
...
foreach $line (@data) {
	...
	$list = "$list<TD><A HREF=?a=photoview&img=$IMG&desc=$DESC TARGET=_img><IMG SRC=$IMG WIDTH=100 HEIGHT=100 BORDER=0></A></TD>";
...
if ($types eq "1") {
$lead = "$lead<TR><TD COLSPAN=2><A HREF=$cgipath/?a=view&user=$usr id=text2>$usr $sir</A></TD>";
...
if ($types eq "2") {
$lead = "$lead<TR><TD></TD>";
$lead = "$lead<TD COLSPAN=2><A HREF=$cgipath/?a=view&user=$usr id=text2>$usr $sir</A></TD>";
...
$messlist = "$messlist<TD><A HREF=?a=Reply&name=$INPUT{'name'}&sender=$usr id=text3>Send</A></TD>\n";
$messlist = "$messlist</TR>\n";
}
...
$cont = "$cont<TABLE BORDER=0 id=table>";
$cont = "$cont<TR><TD><div id=text3>Users name</DIV></TD><TD><div id=text3>Zone Name</DIV></TD><TD><div id=text3>Zone Level</DIV></TD><TD><div id=text3>Amount of items</DIV></TD><TD><div id=text3>Items Needed</DIV></TD><TD><div id=text3>Delete</DIV></TD></TR>";
open (DATA, "raid.lst");
@data = <DATA>;
close DATA;
foreach $line (@data) {
...
$cont = "$cont<TR><FORM><TD><div id=text3>$usr</DIV></TD><TD><div id=text3>$zone</DIV><INPUT TYPE=hidden name=zone value=\"$zone\"></TD><TD><div id=text3>$zlvl</DIV></TD><TD><div id=text3>$zamt</DIV></TD><TD><div id=text3>$zitems</DIV></TD><TD><INPUT TYPE=SUBMIT NAME=a VALUE=deleteraid id=input></TD></FORM></TR>";
}
$cont = "$cont</TABLE>";
...
($first, $sur, $other) = split(/ /, $INPUT{'name'});
if ($first eq "") { 
$first = $INPUT{'name'};
}
...
print MAIL "Welcome To $guildname $name\n";
...
<LINK REL="STYLESHEET" TYPE="text/css" HREF="../style/$style/style.css">
  <TITLE>$guildname > Guild Vault</TITLE>
</HEAD>
<BODY>
<div id="wrapper">
<div id="heading">
...
<div id="pages"><A HREF="index.cgi" id="pageslink">$guildname</A> > Guild Vault</div>
<CENTER>
HTML
...
close DATA;
foreach $line (@data) {
($name, $slot, $magic, $skill, $delay, $dmg, $ac, $wt, $str, $sta, $agi, $dex, $wis, $int, $cha, $svp, $svm, $svd, $svf, $svc, $effect, $smod, $class, $race, $price, $qty) = split(/::/, $line);

print <<"HTML";
<TABLE BORDER=0 CELLPADDING="10" WIDTH="320" BACKGROUND="fullback.jpg">
  <TR>
    <TD colspan="3" valign="top" width="500"><SMALL>$name<BR>
HTML
if ($magic eq "magic") {
print "      MAGIC<BR>";
}
...
print"      <BR>";
if ($effect ne "") {
print "      Effect: $effect<BR>";
}
if ($smod ne "") {
print "      Skill Mod: $smod<BR>";
}
...
<FORM>
  <CENTER>
	...
	<TD COLSPAN=6>
      ...
      <TR>
	<TD COLSPAN=6>
	  <INPUT TYPE="checkbox" NAME="magic" VALUE="magic">Magic Item</TD>
      </TR>
      <TR>
	<TD>Slot:</TD>
	<TD COLSPAN=3>
	  <INPUT TYPE="text" NAME="slot"></TD>
	<TD></TD>
	<TD></TD>
      </TR>
      <TR>
	...
	<TD COLSPAN=6><P ALIGN=Center>
	  Price: <INPUT TYPE=text name="price"></TD>
      </TR>
      <TR>
	<TD COLSPAN=6><P ALIGN=Center>
	  Guild Owns: <INPUT TYPE=text name="qty"></TD>
      </TR>
      <TR>
	<TD COLSPAN=6><P ALIGN=Center>
	  <INPUT TYPE=submit VALUE="Add_Item" name="a"> &nbsp;
	  <INPUT TYPE=reset></TD>
      </TR>
    </TABLE>
  </CENTER>
...
<FORM>
  <CENTER>
	...
	<TD>Slot:</TD>
	<TD COLSPAN=3>
	  <INPUT TYPE="text" NAME="slot" VALUE="$slot"></TD>
	<TD></TD>
	<TD></TD>
      </TR>
      <TR>
	<TD>Skill:</TD>
	<TD COLSPAN=3>
	  <INPUT TYPE="text" NAME="skill" VALUE="$skill"></TD>
	<TD>Delay:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="delay" SIZE="3" VALUE="$delay"></TD>
      </TR>
      <TR>
	<TD>Dmg:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="dmg" SIZE="3" VALUE="$dmg"></TD>
	<TD>AC:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="ac" SIZE="3" VALUE="$ac"></TD>
	<TD>WT:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="wt" SIZE="3" VALUE="$wt"></TD>
      </TR>
      <TR>
	<TD>STR:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="str" SIZE="3" VALUE="$str"></TD>
	<TD>STA:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="sta" SIZE="3" VALUE="$sta"></TD>
	<TD>AGI:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="agi" SIZE="3" VALUE="$agi"></TD>
      </TR>
      <TR>
	<TD>DEX:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="dex" SIZE="3" VALUE="$dex"></TD>
	<TD>WIS:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="wis" SIZE="3" VALUE="$wis"></TD>
	<TD>INT:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="int" SIZE="3" VALUE="$int"></TD>
      </TR>
      <TR>
	<TD>CHA:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="cha" SIZE="3" VALUE="$cha"></TD>
	<TD>SVP:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="svp" SIZE="3" VALUE="$svp"></TD>
	<TD>SVM:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="svm" SIZE="3" VALUE="$svm"></TD>
      </TR>
      <TR>
	<TD>SVD:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="svd" SIZE="3" VALUE="$svd"></TD>
	<TD>SVF:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="svf" SIZE="3" VALUE="$svf"></TD>
	<TD>SVC:</TD>
	<TD>
	  <INPUT TYPE="text" NAME="svc" SIZE="3" VALUE="$svc"></TD>
      </TR>
      <TR>
	<TD>Effect:</TD>
	<TD COLSPAN=5>
	  <INPUT TYPE="text" NAME="effect" VALUE="$effect"></TD>
      </TR>
      <TR>
	<TD>Skill Mod:</TD>
	<TD COLSPAN=5>
	  <INPUT TYPE="text" NAME="smod" VALUE="$smod"></TD>
      </TR>
      <TR>
	<TD>Class:</TD>
	<TD COLSPAN=5>
	  <INPUT TYPE="text" NAME="class" VALUE="$class"></TD>
      </TR>
      <TR>
	<TD>Race:</TD>
	<TD COLSPAN=5>
	  <INPUT TYPE="text" NAME="race" VALUE="$race"></TD>
      </TR>
      <TR>
	<TD COLSPAN=6><P ALIGN=Center>
	  Price: <INPUT TYPE=text name="price" VALUE="$price"></TD>
      </TR>
      <TR>
	<TD COLSPAN=6><P ALIGN=Center>
	  Guild Owns: <INPUT TYPE=text name="qty" VALUE="$qty"></TD>
      </TR>
      <TR>
	<TD COLSPAN=6><P ALIGN=Center>
	  <INPUT TYPE=submit VALUE="Save_Valt_Item" name="a"></TD>
      </TR>
    </TABLE>
  </CENTER>
</FORM>
...
if ($name eq "$INPUT{'name'}") {
print DATA "$INPUT{'name'}::$INPUT{'slot'}::$INPUT{'magic'}::$INPUT{'skill'}::$INPUT{'delay'}::$INPUT{'dmg'}::$INPUT{'ac'}::$INPUT{'wt'}::$INPUT{'str'}::$INPUT{'sta'}::$INPUT{'agi'}::$INPUT{'dex'}::$INPUT{'wis'}::$INPUT{'int'}::$INPUT{'cha'}::$INPUT{'svp'}::$INPUT{'svm'}::$INPUT{'svd'}::$INPUT{'svf'}::$INPUT{'svc'}::$INPUT{'effect'}::$INPUT{'smod'}::$INPUT{'class'}::$INPUT{'race'}::$INPUT{'price'}::$INPUT{'qty'}\n";
}
else {
print DATA "$line";
}
...
print <<"HTML";
<FORM>
<TR><TD>$name<input type=hidden name=name value="$name"</TD>
<TD><INPUT TYPE=submit VALUE="Edit_Valt_Item" name=a></TD>
<TD><INPUT TYPE=submit VALUE="Delete_from_Valt" name=a></TD></TR>
</FORM>
HTML
}
print <<"HTML";
	<TD COLSPAN=3><P ALIGN=Center>
<A HREF="?a=valtadd" id=link>Add Item</a>
</TD>
      </TR>
...
$cont = "$cont Photo added to album";
...
open (DATA, "msg/msges/$INPUT{'msgnum'}");
@data = <DATA>;
close DATA;
...
open(DATA, ">msg/count.txt");
print DATA "$abcdefg\n";
print DATA "junk\n";
close DATA;


open (DATA, "msg/dat/$INPUT{'sender'}.msg");
@data = <DATA>;
close DATA;
...
$message = "<A HREF=?a=list id=message>You have $count Messages</A>";
}
####ENDMESSAGE
if ($style eq "") {
$style = "Defalt";
...
if ($acount eq "Silver") {
$banner = "<div id=text3>Powered by <A HREF=http://www.eqguilded.com id=text3>EQ Guilded</A></DIV>";

$buygold = "Get a Gold Acount now<BR><A HREF=http://www.dotguild.com id=text3>Click here to learn how</A>";
}
...
$file_content =~ s/!!image!!/$INPUT{'img'}/gi;
...
$file_content =~ s/!!css!!/$css/gi;
$file_content =~ s/!!sndmesslist!!/$messlist/gi;
$file_content =~ s/!!msglist!!/$file_content_a/gi;
$file_content =~ s/!!msgview!!/$mymsg/gi;
$file_content =~ s/!!sendto!!/$INPUT{'sender'}/gi;
$file_content =~ s/!!stylesheet!!/$style/gi;
$file_content =~ s/!!banner!!/$banner/gi;
$file_content =~ s/!!acount!!/$acount/gi;
$file_content =~ s/!!buygold!!/$buygold/gi;
close (FILE);
