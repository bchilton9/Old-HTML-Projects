...
# forum.pl                                                                    #
# v0.9.8                                                                      #
# Date Formatting and Timezones by: Floyd for v0.9.8                          #
#                                                                             #
...
################
	...
	foreach $curcat (@categories) {
		...
		$catinfo[1] =~ s/[\n\r]//g;
		if ($catinfo[1] ne "") {
			if ($settings[7] ne "$root" && $settings[7] ne "$catinfo[1]" && $settings[7] ne "Officer") {	next; }
		}
...
if ($settings[7] eq "$root" || $username eq "$boardmoderator") { print qq~
&nbsp;&nbsp;
...
print qq~
<td colspan="2" align="right" valign="top" class="forumtext"><form onSubmit="submitonce(this)" action="$cgi" method="get">$msg{'104'} 
